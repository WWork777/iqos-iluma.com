...
const CheckoutPage = () => {
  ...
  const validateForm = () => {
    ...
    // Валидация только телефона (обязательное поле)
    ...
    // Валидация только Telegram (если указан, необязательное поле)
    if (
  ...
  const handleSubmit = async (e) => {
    ...
    // Проверяем только телефон и Telegram (если указан)
    if (!validateForm()) {
      setLoading(false);
      return;
    }
    ...
    // Определяем тип сообщения на основе города и способа доставки
    let mess = "";
    const isMoscowCity = moscowCities.some((city) =>
      formData.city.trim().toLowerCase().includes(city.toLowerCase()),
    );
...
Общая сумма: ${totalPrice} ₽
      ...
      // ГАРАНТИРОВАННО отправляем в Telegram (основное требование)
      ...
      try {
        ...
        if (telegramResponse.ok) {
          ...
          // Даже если ошибка, продолжаем - главное что попытка была
        }
      } catch (telegramError) {
      ...
      // Сохраняем заказ в базу данных (необязательно, но пытаемся)
      try {
        ...
        // Продолжаем даже при ошибке базы
      }

      // Отправляем в WhatsApp (необязательно)
      if (
        ...
        try {
          ...
          // Продолжаем даже при ошибке WhatsApp
        }
      }
      ...
      // Даже при ошибке показываем успешное сообщение
      alert(
        "Ваш заказ был отправлен!\nВ ближайшее время с вами свяжется наш менеджер.",
      );
      window.location.href = "/";
