...
const TereaCountry = ({
  ...
  const handleClick = (id) => {
    setSelectedCountries((prevSelected) => {
      if (prevSelected.includes(id)) {
        // Если кликнули на уже выбранную страну, снимаем выбор
        return [];
      } else {
        // Выбираем только эту страну (заменяем предыдущий выбор)
        return [id];
      }
    });
  };
  ...
  useEffect(() => {
    // Восстанавливаем активную страну из selectedCountries
    if (selectedCountries.length > 0) {
      setActiveId(selectedCountries[0]);
    } else {
      setActiveId(null);
    }
  }, [selectedCountries]);
          ...
          {Countries.map((item) => (
            <SwiperSlide key={item.id}>
              <button
                ...
                className={`custom-button ${
                  selectedCountries.includes(item.id) ? "active" : ""
                }`}
              >
                {item.title}
              </button>
